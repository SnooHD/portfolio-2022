...
<template>
  <div
    ...
    class="font-merriweather text-white text-[18px] leading-[21px] h-[50%] pt-[200px] xs:pt-[120px]"
    :style="{
      ...
      <li v-for="({ link, icon, text }, index) in socials" :key="`intro-social-list-${index}`">
        <a :href="link" target="_blank" class="group inline-block">
          <Text>
            ...
            >
              <div
                class="transition-transform duration-300 inline-block group-hover:translate-y-[-3px]"
              >
                {{ text }}
              </div>
            </div>
